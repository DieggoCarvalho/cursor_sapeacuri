DECLARE
	...
	@DAJ_COD_VALIDACAO VARCHAR(25), @DAJ_VALOR_ATO NUMERIC(16,2), @DAJ_OBSERVACAO VARCHAR(200), @DAJ_EMISSOR VARCHAR(5), @DAJ_PGE NUMERIC(16,2),
...
1 DAJ_QTD,
NULL DAJ_USUARIO,
--(Select PES_COD from IMOVEIS..GER_PESSOA where PES_COD_OLD = d.fk_id_pessoa and ENT_CODIGO = 1061) PES_COD_CONTRIBUINTE,
...
CAST(d.vl_ato AS NUMERIC(16,2)) DAJ_VALOR_ATO,
d.tx_observacao DAJ_OBSERVACAO,
d.fk_cd_emissor DAJ_EMISSOR,
CAST(d.vl_pge AS NUMERIC(16,2)) DAJ_PGE,
CAST(d.vl_fmmpba AS NUMERIC(16,2)) DAJ_FMMPBA,
CAST(CAST(d.id_daje AS BIGINT) AS VARCHAR(25)) DAJ_CODIGO_OLD,
d.is_isento DAJ_ISENTO,
...
Case
	...
	End DAJ_STATUS,
Case 
	When d.is_bloqueado = 0 Then 'false'
	When d.is_bloqueado = 1 Then 'true'
	Else NULL
	End DAJ_BLOQUEADO
FROM
PALMEIRAS_GERAL..cg_daje d 
left join PALMEIRAS_GERAL..cg_ato a on a.id_ato = d.fk_id_ato
left join PALMEIRAS_GERAL..cg_tipo_ato ta on ta.id_tipo_ato = a.fk_id_tipo_ato
left join PALMEIRAS_GERAL..cg_atribuicao at on a.fk_id_atribuicao = at.id_atribuicao
left join PALMEIRAS_GERAL..cg_selo s on s.id_selo = d.fk_id_selo 
...
OPEN CURSO_PADRAO
 FETCH NEXT FROM CURSO_PADRAO INTO
	...
	@DAJ_COD_VALIDACAO, @DAJ_VALOR_ATO, @DAJ_OBSERVACAO, @DAJ_EMISSOR, @DAJ_PGE,
